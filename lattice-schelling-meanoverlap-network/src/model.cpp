...
#define SVN_VERSION_MODEL_CPP "Schelling segregation plus network (Neal&Neal) model mean overlap rev 4"
...
// stopping criterion for Schelling segregation model with mean overlap
// (for multivariate types): mean cultural overlap over neighbours is
// at at least the tolerance threshold sigma
bool all_happy(Grid<int >&L, Grid<int>& O, Grid<int>& C, int n, int m, int F, int moore_radius, double sigma) {
  for (int i = 0; i < n; i++){
    bool happy = (mean_overlap_neighbours(L, O, C, m, F, i, moore_radius) >= sigma);
    if (!happy) {
      return false;
    }
  }
...
//   given the tolerance threshold sigma
//   randomly move agents to vacant point until each agent is 'happy'
//   because the average cultural similarity of its neighbours is at least sigma
// 
unsigned long long schelling(Grid<int>& L, Grid<int>& O, Grid<int>& C,
  ...
  int nextstep = 0;
    ...
    for (unsigned long long t = 0; t < (unsigned long long) tmax; t++) {
	  ...
	  C.write((toString(tempfileprefix) + "-" + toString(timesteps[nextstep]) + ".C").c_str(), ',');
    	...
    	if (t == 50000 || t == 100000 || t == 500000 || t == 1000000 || (t > 0 && t % 10000000 == 0)) {
    		std::cout << "Reaching " << t << " iterations." << std::endl;
			  ...
			  if (all_happy(L, O, C, n, m, F, moore_radius, sigma)) {
      ...
      bool happy = (mean_overlap_neighbours(L, O, C, m, F, a, moore_radius) >= sigma);
      if (!happy) {
...
int main(int argc, char* argv[]) {
  ...
  double sigma = 0.5;
    ...
    sigma = atof(argv[index++]); // mean cultural similarity tolerance threshold (T in Gracia-Lazaro et al 2009)
	}
  ...
  unsigned long long tend = schelling(L, O, C, sigma, 
                                      tmax, n, m, F, q, 
